      ...
      <el-row v-loading="loading">
        ...
        <el-col :span="13" class="pr">
          <el-card shadow="hover" class="insurance-detail">
            <div class="title">
              <h2 v-if="info.product">{{info.product.name}}</h2>
              <p>保单号：{{info.warranty_code}}</p>
              <p>订单号：{{info.warranty_uuid}}</p>
            </div>
            <el-form label-width="100px" class="list-detail">
              <el-form-item label="公司名称：">
                <p v-if="info.product">{{info.product.company_name}}</p>
              </el-form-item>
              <el-form-item label="保险险种：">
                <p v-if="info.product">{{info.product.category_name}}</p>
              </el-form-item>
              <el-form-item label="条款：">
                <div class="form-inside-row" v-if="info.product && info.product.clause!='[]'">
                  <el-row v-for="item in info.product.clause" :key="item.file_url"><a :href="item.file_url">{{item.detail}}</a></el-row>
                </div>
              </el-form-item>
              <!-- <el-form-item label="责任保额：">
                <el-row type="flex" justify="space-between">
                  <el-col :span="16">意外身故/伤残</el-col>
                  <el-col :span="6">1000</el-col>
                </el-row>
              </el-form-item> -->
              <el-form-item label="受益人：">
                <div v-for="item in info.person" :key="item.name" v-if="item.type == 3"><p v-if="item.name">{{item.name}}</p><p v-else>-</p></div>
              </el-form-item>
              <el-form-item label="保费：">
                <p v-for="item in info.cost" :key="item.id">{{item.premium}}元</p>
              </el-form-item>
              <el-form-item label="支付方式：">
                <p v-for="item in info.cost" :key="item.id">{{constType.pay_way[item.pay_way]}}</p>
              </el-form-item>
              <el-form-item label="支付金额：">
                <p v-for="item in info.cost" :key="item.id">{{item.pay_money}}</p>
              </el-form-item>
              <el-form-item label="支付时间：">
                <p v-for="item in info.cost" :key="item.id">{{formatTime(item.actual_pay_time)}}</p>
              </el-form-item>
              <el-form-item label="缴费期限：">
                <p v-for="item in info.cost" :key="item.id">{{item.phase?item.phase+'年':'暂无'}}</p>
              </el-form-item>
              <el-form-item label="代理人：">
                <p v-if="info.agent">{{info.agent.name}}</p><p v-else>-</p>
              </el-form-item>
              <el-form-item label="代理人推广费：">
                <p v-if="info.product">{{info.product.base_ratio}}%</p><p v-else>-</p>
              </el-form-item>
        ...
        <el-col :span="11">
          <!-- 投保人 -->
          <template>
            <el-card v-for="item in info.person" :key="item.name" shadow="hover" class="text-card">
              ...
              <el-row v-show="item.type==1?insureIsShow:(item.type==2?beInsureedIsShow:benefitIsShow)" class="custom-data">
                <el-col>
                  <el-form label-width="100px" class="list-small-detail inline">
                    ...
                    <el-form-item label="与被保人关系：">{{formatName(item.relation_name)}}
                    </el-form-item>
...
import { getDetail } from '@/api/warranty'
...
export default {
  ...
  methods: {
    ...
    goBack() {
      this.$router.back(-1)
      // this.$router.push({ path: '/warranty/list' })
    }
  },
  created() {
    this.getCurInfo()
